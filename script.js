...
// Global modal navigation tracking
let cameFromDashboardFolder = false;
...
function closeProjectModal() {
    ...
    if (modal) {
        ...
        if (cameFromDashboardFolder) {
            cameFromDashboardFolder = false;
            openDashboardModal();
        }
    }
}

// EDA Folder Modal Logic
function openEdaModal() {
    const modal = document.getElementById('eda-folder-modal');
    if (modal) {
        modal.classList.add('active');
        document.body.style.overflow = 'hidden'; // Lock scroll
        lucide.createIcons();
...
// Dashboard Folder Modal Logic
function openDashboardModal() {
    const modal = document.getElementById('dashboard-folder-modal');
    if (modal) {
        modal.classList.add('active');
        document.body.style.overflow = 'hidden'; // Lock scroll
        lucide.createIcons();
...
window.addEventListener('click', (e) => {
    const modal1 = document.getElementById('project-modal');
    if (e.target === modal1) {
        closeProjectModal();
    }
    const modal2 = document.getElementById('eda-folder-modal');
    if (e.target === modal2) {
        closeEdaModal();
    }
    const modal3 = document.getElementById('dashboard-folder-modal');
    if (e.target === modal3) {
        closeDashboardModal();
    }
});
